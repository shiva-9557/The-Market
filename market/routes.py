...
from market.forms import RegisterForm, LoginForm, PurchaseItemForm, SellItemForm, addItemForm,editItemForm
...
def new_item():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f"{item_to_create.name} is successfully added", category='success')
        return redirect(url_for('market_page'))
    if form.errors != {}: #If there are not errors from the validations
        for err_msg in form.errors.values():
...
def edit_item():
    ...
    item_to_edit=Item.query.filter_by(id=itemid).first()
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f"{item_to_edit.name} is successfully edited", category='success')
        return redirect(url_for('market_page'))
    if form.errors != {}: #If there are not errors from the validations
        for err_msg in form.errors.values():
            flash(f'There was an error with editing a item: {err_msg}', category='danger')

    return render_template('edititem.html', form=form,it=item_to_edit)
